...
public class CarServiceMain {
	public static void main(String[] args) {
		...
		int parkno = carService.getTotalParkingNumber();
		System.out.println("전체주차구획수 : "+ parkno);
		System.out.println("3.CarService객체야 주차가능주차구획수반환해줘");
		int available = carService.getAvailableParkingNumber();
		System.out.println("주차가능구획수 : "+available);
		System.out.println("4.CarService객체야 차량번호(2322번) 인자로줄께  차객체한대 참조변수반환해줘");
		carService.findByNo("2322").print();;
		
		System.out.println("5.CarService객체야 입차시간(8시이후) 인자로줄께 차객체배열 참조변수반환해줘");
		Car[] findtime = carService.findByInTime(8);
		for (Car car : findtime) {
			car.print();
		}
		System.out.println("6.CarService객체야 차량번호(1212번) 출차시간(12시)인자로줄께 출차시켜주고 출차차량참조변수반환해줘");
		Car find = carService.chulCha("1212", 12);
		find.print();
	}
